...
const UserModel = {
  ...
  getActiveUsersCount: async () => {
    const [rows] = await db.query(
      `SELECT COUNT(*) as count FROM users_new WHERE status = 'active'`
    );
    return rows[0].count;
  },
  getBlockedUsersCount: async () => {
    const [rows] = await db.query(
      `SELECT COUNT(*) as count FROM users_new WHERE status = 'blocked'`
    );
    return rows[0].count;
  },
  getAuthorUsersCount: async () => {
    const [rows] = await db.query(
      `SELECT COUNT(*) as count FROM users_new WHERE role = 'author'`
    );
    return rows[0].count;
  },
  ...
  updateUser: async (id, updatedData) => {
    ...
    const fields = fieldsToUpdate.map(([key]) => `${key} = ?`);
    ...
    try {
      ...
      console.error("Lỗi trong UserModel.updateUser:", dbError);
      throw dbError;
    }
  },
